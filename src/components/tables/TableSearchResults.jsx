...
import { formataData, formataHora } from "../../util/util";

const TableSearchResults = ({ transactions, onEdit, onDelete }) => {
        ...
        <Table.Body className="divide-y">
          {transactions.map((transaction) => (
            <Table.Row className="bg-white" key={transaction.id}>
              <Table.Cell className='text-slate-950'>
                {formataData(new Date(transaction.createdAt))}
              </Table.Cell>
              <Table.Cell className='text-slate-950'>
                {formataHora(transaction.createdAt)}
              </Table.Cell>
